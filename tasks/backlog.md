...
- **notes:** **DONE 23 Jun.** Rebuilt the SI React app onto a hardened Azure data layer: JSON-Schema-2020-12-validated engagement datasets (framework snapshotted per engagement), Azure Functions API (schema + cross-ref gated CRUD, multi-dataset), app wired to it (engagement picker → hydrate → debounced autosave — fixes the no-persistence blocker). Deployed as Azure **Static Web App** (repo `WWRI-Structured-Interviews`, RG WWRI-RPs) against existing **wwridatabase** storage (container `wwri-si`). **LIVE: https://red-pond-0da017a00.7.azurestaticapps.net** — full public CRUD verified end-to-end. **Remaining:** rotate wwridatabase key1 (shared in-session) + update SWA env; browser acceptance test; hand URL to Henri. See memory structured-interview-app. **24 Jun (email re-audit + diagnosis):** Niel tested, liked the setup flow, but reported an error on "result" + a persistence/back question. **Diagnosed:** Niel's screenshot was a **401 Unauthorized on a different, auth-gated app** (`identity.7.azurestaticapps.net`), NOT this SI tool — the SI app (`red-pond-0da017a00.7.azurestaticapps.net`) is verified **public** (HTTP 200, no `.auth` redirect, `/api/engagements` 200) and its data layer validates cleanly against the hardened schema for every shape the app emits. Likely Niel was sent/using the wrong link. **24 Jun: Angelus sent Niel the correct red-pond link.** **Awaiting Niel to retest Results there.** If it still errors, reopen — fix ready: guard `buildQuestions` (`tc.questions[stId]` → `(tc.questions||{})[stId]`, `st.examples[0]` → `(st.examples||[])[0]`) + add a Results error boundary. Persistence answer for Niel: there IS an Engagements picker + "← Engagements" button; no per-engagement deep link by design. Multi-interviewer collaboration on one project = a real future-phase request from Niel.
  - **TESTED 25 Jun (Niel + Angelus, Wednesday):** app working. Retest confirmed — 401 issue resolved; persistence/layout accepted. **Remaining open work:** (1) interviewer name missing from results page; (2) CSS centring (Setup/Results left-aligned) [DONE 26 Jun — `pageWrap` helper, all tabs centred]; (3) future: import transcript to populate answer blocks (Niel's request, human scoring only).
  - **MIGRATION-DRIVEN APP CHANGES (confirmed 26 Jun, detail in [[DEV-011]] sub-notes):** (a) **topic reassignment** — let the questionnaire author move a question/subtopic to a different topic (corrects grandfathered best-match mappings); (b) **weighted rollups** — `topicScore = Σ(score×w)/Σ(w)`, weight controls in Setup [necessary dev]; (c) **overall = mean of topic scores** (not flat criterion average) to match how the real legacy assessments computed "Overall Change Readiness" [sign-off gate — shifts live numbers slightly]. Schema v1.1 already supports (a)/(b); these are the app-logic + UI follow-ons.
  - **IMPLEMENTED 26 Jun (committed, NOT deployed) — branch `si-v1.1-weighting-migration`.** All three above + per-engagement category consumption are done in `app/index.html` and reviewed clean (validator 21/21; adversarial scoring + regression reviews passed). **Data-loss fix included:** autosave was overwriting the snapshotted per-engagement framework with the in-memory library. **Pending Angelus sign-off to push → SWA auto-deploy** (live-app gate; Niel/Henri use it). Two non-blocking follow-ons noted by review: no in-app "add a new category" affordance (display + benchmark editor scale to N, but can't create a tier in-UI); moving an *unselected* subtopic strands its edited question text (cosmetic, no score impact). Original interviewer-name-on-results bug from the 24 Jun ops meeting still to verify post-deploy.
...
- **notes:** **Future task, logged 2026-06-25.** Sibling to [[DEV-010]] — the SI spoke in the cockpit-SSOT hub-spoke model ([[DEV-007]]). Structured-interview data currently lives in **spread-out bespoke assessment spreadsheets** (the Topics→Eval→Summary→Radar workbooks — see [[structured-interview-workbooks]]). **Discover those workbooks in SharePoint** (`node scripts/sharepoint/sp.mjs search "interview"` / by client folder) and **reconcile them into the new SI-app data format** — the Azure hardened-schema data layer being built for the deployed SI app ([[DEV-006]] / [[structured-interview-app]]). Read-write spoke: same propose→approve→audit reconcile back to the cockpit SSOT. **Open scope (DEV-007 architecture pass):** mapping the heterogeneous legacy workbook layouts onto the hardened SI schema, dedupe/identity across cohorts (mind [[jan-sorcek-vs-soucek]]), and one-time backfill vs ongoing capture. Do not start until the hub-spoke architecture is defined.
  - **SCHEMA BUILT 26 Jun — engagement schema bumped to v1.1 (migration-ready).** Deep-dived SharePoint: ~8 distinct client datasets with full scores (SODIAAL incl. Supervisors, Yoplait, DJP/Dominos Japan, Hart/IndigoHart, BWI, SkyCity, SMEC P1+P4; Teradata/CSC/Alstom older formats). All share Topics→Eval→Summary→Radar, score scale **0.0–1.0**, per-category Industry Adequate/Best benchmarks, final "Concerns" topic. Extended `structured-interview/schema/engagement.schema.json` to **v1.1** (synced to API copy; **21/21 validator tests pass**): added subtopic/criterion **weights** (default 1), **per-engagement categories** (benchmarks no longer fixed to 3 tiers — legacy uses 2/4/single-"All"), `canonicalTopic` cross-engagement tags, `provenance` block, legacy interviewee fields (company/region/function/date), raised `selected` cap 3→24. Wrote `schema/MIGRATION.md` (the extraction-skill contract: subtopic→single-criterion rule, ×100 score conversion, weighted-rollup formulas, per-workbook quirks) + `schema/migrated-example-bwi.json` worked fixture. Recommended migration order: SODIAAL first, then BWI. **Next:** build the extraction skill (one workbook at a time, validate→eyeball→commit).
  - **APP DEV REQUIREMENTS confirmed by Angelus 26 Jun (these are [[DEV-006]] app-side changes the migration forces):** (1) **Topic reassignment / author correction** — because we're grandfathering legacy questions onto best-match topics, the questionnaire author needs an in-app way to **reassign a question (subtopic) to a different topic** and correct the mapping. New Setup-screen capability: move a subtopic between topics (and edit its `canonicalTopic` tag). (2) **Weighting [NECESSARY DEV]** — implement weighted rollups in the app: effective crit weight = `subtopic.weight × crit.weight`; `topicScore = Σ(score×w)/Σ(w)`; expose weight controls in Setup. v1.0 app rollup (`app/index.html` ~L695/L727) is an unweighted flatten — must change. (3) **Overall-score semantics to match real examples** — `personOverall` (~L726) currently averages all criteria; legacy "Overall Change Readiness" = **mean of topic scores** (equal topic weight). Change overall to mean-of-topic-scores so the app reflects how the real assessments were actually computed. **NB this slightly shifts existing live numbers when topics have unequal criterion counts — sign-off gate before changing live semantics.**
  - **BUILT + PROVEN 26 Jun (via the `si-dev-and-schema` workflow, 11 agents).** (a) **`si-migrate` skill** built (`.claude/skills/si-migrate/` — SKILL.md + `extract-workbook.mjs`; committed WWRI Work branch `si-migrate-skill`). Pulls sheets via `sp.mjs`, maps the POSITIONAL WWRI template (Topics col A=id/B=label/C=weight; Eval Data columns matched by subtopic **label**; Tables benchmarks), ratio→0-100 + Excel-date-serial→ISO, gates on real `validateEngagement()` before write. Propose→eyeball→`--write`. (b) **All 3 app dev requirements + a v1.1-consumption gap implemented** in `app/index.html` (committed structured-interview branch `si-v1.1-weighting-migration`, **NOT deployed**): weighted rollups, overall=mean-of-topic-scores, topic-reassignment UI, and per-engagement category consumption (dropped hardcoded `CATS`; **data-loss fix**: autosave was overwriting the snapshotted framework). Verified: validator 21/21, adversarial scoring-math review + regression review both clean. (c) **SODIAAL proven end-to-end:** 6 topics, 9 interviewees, 86% score fill-rate, dates converted, scores verified against raw cells, valid=true; landed locally to `engagements/{uuid}.json` (**gitignored** — real assessment data's canonical home is Azure Blob, not git). **Test-drive caught 2 real mapper bugs** (header-vs-positional Topics parse; Excel date serials), both fixed before any write — the propose-gate working as designed. **NEXT:** (1) push branches + deploy when Angelus signs off (live-app gate); (2) upload SODIAAL to the `wwri-si` Blob; (3) run BWI (exercises weights + 4 tiers + Commentary), then DJP/Hart/SkyCity/SMEC; (4) SODIAAL **Supervisors** is a separate 2nd workbook/run.
